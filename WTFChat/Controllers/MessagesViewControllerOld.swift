...
class MessagesViewControllerOld: JSQMessagesViewController {
    var timer: NSTimer?
    ...
    func setupAvatarImage(name: String, imageUrl: String?, incoming: Bool) -> JSQMessagesAvatarImage {
        ...
        return avatarService.getAvatarImage(name, diameter: diameter)
    }
        ...
        -> NSAttributedString?
    {
        ...
        if (indexPath.row == 0) {
            return timeService.parseTime(message.timestamp)
        } else {
